### Item-based Collaborative filtering

In this part, we will try to use the Item-based Collaborative filtering algorithm to predict scores for a set of unwatched movies by a specific user and recommend some movie to this user. Unlike the User-based CF algorithm, the item-based method looks into the connections between the set of movies the target user has rated and the movies the target user has not interacted yet. This approach compute the similarity between a target movie $j$ and selected $k$ most similar movies $\{i_1,i_2,...,i_k\}$ the target user has rated. By having the corresponding similarities $\{s_{j,1},s_{j,2},...,s_{j,k}\}$ , the prediction is computed as a weight average score to the target movie $ j$.

#### Theory

The key assumption behind this algorithm is that we assume users will always remain their taste on movies.

This whole modelling process can be divided into two steps, namely, Similarity Calculation and Prediction. The similarity computation is the most critical move in the entire modelling which determines if the final results is satisfying or not, and is pre-computed before hand.

##### Basic idea of similarity computation

To compute the similarity of two given movie $i$ and $j$ , by only looking into the co-rated users, a co-occurrence matrix $C$ is first constructed, and each entry in the matrix records the number of users who like both movie $i$ and movie $j$. More details will be explaned in the figure1 (Badrul S et al, 2001, p.6) and figure2 (Liang X, 2012, p.):

![Screen Shot 2019-08-09 at 10.59.55 pm](/Volumes/Guess_What/MIT_coursework/9417/9417-assignment/Item-based Collaborative filtering.assets/Screen Shot 2019-08-09 at 10.59.55 pm-5356110.png)

The figure1 represents the process of selecting co-rated users for movie $i$ and movie $j$. The columns are the movies and the rows are users.

![Screen Shot 2019-08-09 at 11.28.36 pm](/Volumes/Guess_What/MIT_coursework/9417/9417-assignment/Item-based Collaborative filtering.assets/Screen Shot 2019-08-09 at 11.28.36 pm.png)

The figure2 elaborates how the co-occurrence matrix is constructed. Each rows in leftmost part indicates movies of a user, each entry in the matrix in the rightmost part implies the total number of users who interacted with both movie $i$ and movie $j$ .

##### Mathematical knowledge

In the computation of similarities between items, an adjusted cosine similarity method and a weight sum method are applied. The reason to use adjusted cosine instead of basic cosine is that it can offset the drawback of the existing fact that different users have different rating scale. The specific formula applied is as below:
$$
sim(i,j) = \frac{\sum_{u\in U}(R_{u,i}-\overline{R})(R_{u,j}-\overline{R})}{\sqrt{\sum_{u\in U}(R_{u,i}-\overline{R})^2}\sqrt{\sum_{u\in U}(R_{u,j}-\overline{R})^2}}\tag{1}
$$
, where$  \overline{R}$ denotes the average ratings rated by user $u$ .
$$
P_{u,j} = \frac{\sum_{i \in N}S_{i,j}\times R_{u,i}}{\sum_{i \in N}|S_{i,j}|}\tag{2}
$$
, where $j$ is the target movie for prediction, $u$ is the target user and $N$ is the set of movies that user $u$ has rated.

#### Experiment

##### Data Split

In terms of evaluation, the raw dataset is splited into two parts -- train (80%), test (20%). The way to split the dataset is that for each user, 80% of the rated movie will be obtained as training data, and then the remaining 20% rated movie becomes "unrated" movie by this user.

 The idea behind this is that when the train dataset is used to fit the model, and then a list of predicted movies will be recommended which includes the movies in the test dataset. Then the comparison of real ratings and predicted ratings becomes achievable, where the $RMSE$ comes in to play.

##### Performance

Recommended Movie for user 1 and user 2

```
The top 10 recommended movies for user 1:
    movieId                                              title  pred
0   55908.0                         Man from Earth, The (2007)   5.0
1  128360.0                           The Hateful Eight (2015)   5.0
2   26366.0                        Harlan County U.S.A. (1976)   5.0
3    2573.0                                       Tango (1998)   5.0
4    5063.0                              One-Eyed Jacks (1961)   5.0
5    4925.0                        Cheap Detective, The (1978)   5.0
6   26147.0                          Thousand Clowns, A (1965)   5.0
7    8738.0  Woman Is a Woman, A (femme est une femme, Une)...   5.0
8   44943.0                                        9/11 (2002)   5.0
9    6344.0               101 Reykjavik (101 Reykjavík) (2000)   5.0
____________________________________________________________________________________________
The top 10 recommended movies for user 2:
    movieId                                              title      pred
0  160718.0                                       Piper (2016)  5.000000
1   44889.0           Reefer Madness: The Movie Musical (2005)  4.850305
2   58492.0                                 Snow Angels (2007)  4.850305
3    8796.0  Funny Thing Happened on the Way to the Forum, ...  4.850305
4   34164.0                               Happy Endings (2005)  4.850305
5    7122.0                              King of Hearts (1966)  4.850305
6    1734.0            My Life in Pink (Ma vie en rose) (1997)  4.638776
7   33880.0             Me and You and Everyone We Know (2005)  4.638776
8   91483.0                          Bullet to the Head (2012)  4.505340
9   95949.0                    Immature, The (Immaturi) (2011)  4.505340
```

Movies genres recommeded VS Movies genres user 1 rated

```
The genres of movies recommended to this user
0	Comedy	2433
1	Drama	1918
2	Action	1602
3	Adventure	575
4	Crime	471
5	Horror	396
6	Documentary	307
7	Animation	239
8	Children	181
9	Thriller	67
____________________________________________________________________________________________
The genres of movies this user has rated
0	Action	90
1	Comedy	48
2	Adventure	39
3	Drama	17
4	Crime	15
5	Animation	12
6	Horror	7
7	Children	2
8	Mystery	1
9	Thriller	1
```

As showed, the top five genres of movies recommended is just same as the top five genres of movie this user has rated, the order is slightly different, though.

##### Average RMSE perfomance on different K values

After the experiments on a single target user, the scope is changed towards the performance of average RMSE on all users in terms of a range of K values. 

![img](data:image/png;base64,)

The figure indicates a rapid decrease when the K values increase in the starting point, and when the K values get larger, the average RMSE tends to be steadible as anticipation.

##### Cross-validation

A cross-validation is also applied for the measurement of accuracy of this predictive model, and an average RMSE of  0.8634203453665978 and a variance of 0.00011318512712920291 are obtained. This indicates the model is very steadible.

#### Insights & Improvements

While experimenting, some unignorable problems are encountered. 

First, the popular movies seem to have a higher similarity with other movies, which leads to a higher prediction. Eventually, the model will tend to recommend the popular movie but not necessary be interested by the user. Therefore, to tackle this problem, a normalization on the weights is applied when compute the prediction. This improves the average RMSE to 0.890520381305443 with a K value of 10.

Another porblem is that this algorithm can not make predictions on those movies that nobody has ever rated. Then the unpopular but high quality movies are unlikely to be recommended before anyone becomes the first person who eats crab.

### Reference

Badrul, S, George, K, Joseph,K, & John,R 2001, *Item-based Collaborative Filtering Recommendation
Algorithms*, WWW10, Hong Kong

Liang, X 2012,*Recommand System Practice*,Post and Telecom Press, Beijing.